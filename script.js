...
window.onload = () => { 
    ...
    const projSel = document.getElementById('active-project-id');
    if(projSel) projSel.addEventListener('change', handleProjectSelection);
...
function switchPage(pageId, el) {
    ...
    if(titleEl) titleEl.innerText = pageId==='main'?'MAIN_DASHBOARD':(pageId==='terminal'?'TERMINAL_OP_V7.9':'TEMPLATE_MANAGER');
    
    if(pageId === 'terminal') { 
        isSystemAction = true; updateStoreSelect(); 
        if(activeStore) { 
            const pid = document.getElementById('active-project-id'); if(pid) pid.value = activeStore.id;
            const tArea = document.getElementById('active-terminal-area'); if(tArea) tArea.style.display = 'block'; 
...
function editTemplateUI(id) { const t = fixedTemplates.find(x=>x.id==id); if(!t) return; setV('edit-tpl-id', t.id); setV('tpl-day', t.day); setV('tpl-time', t.time||"10:00"); setV('tpl-type', t.type); setV('tpl-amount', t.amount); setV('tpl-category', t.category); setV('tpl-memo', t.memo||""); const sBtn=document.getElementById('tpl-save-btn'); if(sBtn) sBtn.innerText="保存"; const cBtn=document.getElementById('tpl-cancel-btn'); if(cBtn) cBtn.style.display="block"; window.scrollTo({top:0,behavior:'smooth'}); }
function cancelEditTemplate() { ['edit-tpl-id','tpl-day','tpl-amount','tpl-category','tpl-memo'].forEach(k=>setV(k,"")); const sBtn=document.getElementById('tpl-save-btn'); if(sBtn) sBtn.innerText="追加"; const cBtn=document.getElementById('tpl-cancel-btn'); if(cBtn) cBtn.style.display="none"; }
...
function calculateCurrentBudget() {
    ...
    data.forEach(d => { 
        if(d.date>=c.startStr && d.date<c.nextPayStr && d.status!=='deleted' && d.status!=='skipped') { 
            if(d.type==='expense'){ 
                cEx+=d.amount; 
                // まとめ買い（pending）は確定するまで今日/今週のメーターを圧迫させない
...
function addData() { 
    ...
    data.push({ id:Date.now(), date:getV("date"), time:getV("time")||"00:00", timestamp:Date.now(), amount:a, type:getV("type"), category:getV("category"), memo:getV("memo"), actionLogText:"", status:'confirmed' }); 
...
function render() {
    ...
    if(l) {
        data.filter(d=>d.date>=c.startStr && d.date<c.nextPayStr && d.status!=='deleted' && d.status!=='skipped').sort((a,b)=>(b.date+" "+b.time).localeCompare(a.date+" "+a.time)).forEach(d => {
            const p=d.status==='pending', cl=p?`item item-pending`:`item ${d.type}`, bd=p?`<span class="badge-pending">予定</span>`:'', tc=d.type==='expense'?'#ff3b30':'#34c759';
...
function updateMainProgressBar(c) {
    ...
    const u=(bId,vId,aId,bM,cV)=>{ 
        ...
        if(bM<=0&&cV<=0){
            b.style.width='0%'; b.style.background='#1f2937'; v.innerText='0%'; v.style.color='#8e8e93'; a.innerText=textNormal; a.style.color='#8e8e93';
        }else if(cV<0){
...
function showDetail(id) { const d=data.find(x=>x.id===id); if(!d)return; const p=d.status==='pending'; let btns = p ? `<button class="btn btn-green" style="margin-bottom:10px;padding:16px;" onclick="confirmRecord(${id})">✅ 確定にする</button><div style="display:flex;gap:10px;"><button class="btn" style="flex:1;" onclick="updateRecord(${id})">更新</button><button class="btn btn-muted" style="flex:1;" onclick="skipRecord(${id})">スキップ</button></div>` : `<button class="btn" onclick="updateRecord(${id})">保存</button><button class="btn btn-red" onclick="deleteRecord(${id})">削除</button>`; const dc = document.getElementById('detail-content'); if(!dc) return; dc.innerHTML=`<h3 style="margin:0 0 10px;border-bottom:2px solid ${p?'#FF9500':'#007aff'};padding-bottom:5px;">${p?'予定の確認':'データの編集'}</h3><div style="display:flex;gap:10px;"><input type="date" id="edit-date" value="${d.date}" style="flex:2;"><input type="time" id="edit-time" value="${d.time}" style="flex:1;"></div><select id="edit-type" style="margin:10px 0;"><option value="expense" ${d.type==='expense'?'selected':''}>支出</option><option value="income" ${d.type==='income'?'selected':''}>収入</option></select><input type="number" id="edit-amount" value="${d.amount}" inputmode="numeric"><input type="text" id="edit-category" value="${d.category||''}" style="margin:10px 0;"><input type="text" id="edit-memo" value="${d.memo||''}"><textarea id="edit-actionlog" style="font-size:12px;width:100%;height:60px;margin-top:10px;margin-bottom:10px;">${d.actionLogText||''}</textarea>${btns}<button class="btn btn-muted" onclick="document.getElementById('detail-modal').style.display='none'">閉じる</button>`; document.getElementById('detail-modal').style.display='flex'; }
...
function finishProject() { 
    ...
    const area=document.getElementById('active-terminal-area'); if(area) area.style.display='none'; setV('active-project-id', ""); activeStore=null; cancelEditStore(); save(); render(); switchPage('main',document.querySelector('.tab-item')); 
}

...
function calculateCurrentBudget() {
    ...
    data.forEach(d => { 
        if(d.date>=c.startStr && d.date<c.nextPayStr && d.status!=='deleted' && d.status!=='skipped') { 
            // ※まとめ買い除外の pending 判定はまだ入っていない、正常稼働時の状態
    ...
    if(l) {
        ...
        if(filtered.length === 0){
            ...
            filtered.sort((a,b)=>(b.date+" "+b.time).localeCompare(a.date+" "+a.time)).forEach(d => {
                const p=d.status==='pending', cl=p?`item item-pending`:`item`, bd=p?`<span class="badge-pending">予定</span>`:'', tc=d.type==='expense'?'#ff3b30':'#34c759';
                const v=document.createElement("div"); v.className=cl; v.innerHTML=`<div><small style="color:#9ca3af;display:block;">${d.date} ${d.time}</small>${bd}${d.category||'未分類'} <small style="color:#64748b;">${d.memo?'('+d.memo+')':''}</small></div><div style="color:${p?'#8e8e93':tc};font-weight:bold;">${d.type==='expense'?'-':'+'}${d.amount.toLocaleString()}円</div>`; v.onclick=()=>showDetail(d.id); l.appendChild(v);
            });
        }
    }
...
function updateMainProgressBar(c) {
    ...
    const u=(bId,vId,aId,bM,cV)=>{ 
        ...
        if(bM<=0&&cV<=0){
            b.style.width='0%'; b.style.background='#1f2937'; v.innerText='0%'; v.style.color='#9ca3af'; a.innerText=textNormal; a.style.color='#9ca3af';
        }else if(cV<0){
            ...
            b.style.width=p+'%'; v.innerText=Math.floor(p)+'%'; v.style.color='#ffffff';
            b.style.background=p>50?'#34C759':(p>20?'#FFCC00':'#FF3B30'); a.innerText=textNormal; a.style.color='#9ca3af';
        }
    };

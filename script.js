...
function updateMainProgressBar() {
    ...
    if (!isTerm) {
        // ★修正: MAIN画面の予算防衛メーターを「残量（RPGのHPバー）」方式に統一
        const uMain = (bId, vId, aId, maxVal, remainVal) => { 
            ...
            if (maxVal <= 0 && remainVal <= 0) {
                ...
                a.innerText = '残 0円 / 予算 0円'; a.style.color='#8e8e93';
            } else if (remainVal < 0) {
                b.style.width='0%'; // バーはゼロ（シールド消失）
                b.style.background='repeating-linear-gradient(45deg,#ff3b30,#ff3b30 8px,#ff6b6b 8px,#ff6b6b 16px)'; 
                v.innerText='OVER'; v.style.color='#ff3b30'; a.innerText=`予算超越 (赤字 ${Math.abs(remainVal).toLocaleString()}円 / 予算 ${maxVal.toLocaleString()}円)`; a.style.color='#ff3b30';
            } else {
                let p = maxVal > 0 ? (remainVal / maxVal) * 100 : 100;
                ...
                a.innerText = `残 ${remainVal.toLocaleString()}円 / 予算 ${maxVal.toLocaleString()}円`; a.style.color='#1c1c1e';
            }
        };
        ...
        // ★修正: TERMINAL画面のメーターを「REM（残量）」ベースに統一し、防衛シールドらしく
        const updateTermMeter = (bId, vId, aId, maxVal, usedVal, pendingVal, unit, name) => {
            let b=document.getElementById(bId), v=document.getElementById(vId), a=document.getElementById(aId);
            if(!b || !v || !a) return;
            
            let totalUsed = usedVal + pendingVal;
            let remain = maxVal - totalUsed;
            if (maxVal <= 0) {
                b.style.width='0%';
                b.style.background='#e5e5ea'; v.innerText='0%'; v.style.color='#8e8e93'; 
                a.innerText = `REM: 0${unit} / MAX: 0${unit}`; a.style.color='#8e8e93';
            } else if (remain < 0) { // オーバー
                b.style.width='0%'; // バーはゼロ
                b.style.background='repeating-linear-gradient(45deg,#ff3b30,#ff3b30 8px,#ff6b6b 8px,#ff6b6b 16px)'; 
                v.innerText='OVER'; v.style.color='#ff3b30'; a.innerText=`REM: ${remain.toLocaleString()}${unit} / MAX: ${maxVal.toLocaleString()}${unit} (OVER)`; a.style.color='#ff3b30';
            } else {
                let p = (remain / maxVal) * 100;
                if(p < 0) p = 0; if (p > 100) p = 100;
                b.style.width = p + '%'; 
                v.innerText = Math.floor(p) + '%'; v.style.color='#1c1c1e';
                b.style.background = p > 50 ? '#34C759' : (p > 20 ? '#FFCC00' : '#FF3B30');
                a.innerText = `REM: ${remain.toLocaleString()}${unit} / MAX: ${maxVal.toLocaleString()}${unit}`; a.style.color='#1c1c1e';
            }
        };
